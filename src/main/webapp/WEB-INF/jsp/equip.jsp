...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        ...
        <a href="${pageContext.request.contextPath }/equip/addequip.action" class="btn btn-primary">新建</a>
            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="btn btn-primary btn-xs" data-toggle="modal"
                                       data-target="#EquipEditDialog" onclick="editEquip(${row.ecode})">修改</a>
                                    <a href="#" class="btn btn-danger btn-xs" onclick="deleteEquip(${row.ecode})">删除</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    ...
                    <div class="col-md-12 text-right">
                        <kahen:page url="${pageContext.request.contextPath }/equip/list.action"/>
                    </div>
                    <!-- /.panel-body -->
                </div>
                <!-- /.panel -->
                    ...
                    <div class="panel panel-default">
                        <form id="edit_equip_form" class="form-group">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" onclick="updateEquip()">保存修改</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script type="text/javascript">
    function editEquip(ecode) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#enumber").val(data.enumber);
            }
        });
    }

    function updateEquip() {
        $.post("<%=basePath%>equip/updateequip.action", $("#edit_equip_form").serialize(), function (data) {
            if (data == "OK") {
                alert("材料信息更新成功！");
    ...
    // 删除材料
    function deleteEquip(ecode) {
        if (confirm('确实要删除该材料吗?')) {
            $.post("<%=basePath%>equip/deleteequip.action", {"ecode": ecode},
                function (data) {
                    if (data == "OK") {
                        alert("材料删除成功！");
                        window.location.reload();
