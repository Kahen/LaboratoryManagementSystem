            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="btn btn-primary btn-xs" data-toggle="modal"
                                       data-target="#AdminEditDialog" onclick="PassLab(${row.lcode})">通过</a>
                                    <a href="#" class="btn btn-danger btn-xs" onclick="RejectLab(${row.lcode})">拒绝</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    ...
                    <div class="col-md-12 text-right">
                        <kahen:page url="${pageContext.request.contextPath }/apllab/list.action"/>
                    </div>
                    <!-- /.panel-body -->
                </div>
                <!-- /.panel -->
...
<script type="text/javascript">
    // 通过实验室申请
    function PassLab(lcode) {
        if (confirm('确实要通过该申请吗?')) {

            $.post("<%=basePath%>lab/passlab.action", {"lcode": lcode},
                function (data) {
                    if (data == "ERROR") {
                        alert("实验室未在审核中");
    ...
    // 拒绝实验室申请
    function RejectLab(lcode) {
        if (confirm('确实要拒绝该申请吗?')) {
            $.post("<%=basePath%>lab/rejectlab.action", {"lcode": lcode},
                function (data) {
                    if (data == "ERROR") {
                        alert("实验室未在审核中");
                        window.location.reload();
